...
" 2>/dev/null || echo "")
      ...
      # Check ALL commits on this branch for hygiene violations
      # First try with base ref, then fall back to checking all branch commits
      HYGIENE_VIOLATIONS=""
      VIOLATION_MESSAGES=""
      ...
      if [ -n "$BASE_REF" ]; then
        ...
        # No base ref: check all commits on current branch (last 20)
        BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "HEAD")
        HYGIENE_VIOLATIONS=$(git log --format="%s%n%b" -n 20 2>/dev/null | grep -ciE '(^WIP|^debug|^temp|^fixup|^squash|Co-authored-by|Generated by|Generated with|Claude Code|Anthropic|ChatGPT|GPT-|Copilot)' || true)
        VIOLATION_MESSAGES=$(git log --format="%h %s" -n 20 2>/dev/null | grep -iE '(^WIP|^debug|^temp|^fixup|^squash|Co-authored-by|Generated by|Generated with|Claude Code|Anthropic|ChatGPT|GPT-|Copilot)' || true)
      fi

      if [ -n "$HYGIENE_VIOLATIONS" ] && [ "$HYGIENE_VIOLATIONS" -gt 0 ]; then
