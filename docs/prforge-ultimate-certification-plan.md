...
**2026-05-08 — Observation-derived regression:** During dogfooding, we observed an agent willing
to mark self-review `APPROVE` while acknowledging a core quality weakness in the same artifact:
`"Known tradeoff: LLM synthesizes gracefully from empty results. Accepted for v1."` This language
passed no enforcement gate and would have reached a public push unblocked.

PRForge now includes a generic quality weakness gate (`scripts/quality_weakness_gate.py`) that
blocks this class of failure before PACKAGE and before public approval.

Proof record:
- `quality_weakness_gate.py` direct invocation blocks the artifact (exit 2, 4 findings with artifact:line)
- `phase-boundary.sh` blocks SELF_REVIEW→PACKAGE via hook (the write is rejected before state advances)
- `pr_approve.py` blocks public action even if phase is manually forced to APPROVAL
- Regression tests 16–20 in `test_prforge_regressions.sh` cover all three enforcement paths deterministically
...
| Hooks smoke | `bash scripts/tests/hooks/test_prforge_regressions.sh` | 20/20 PASS |
| Preflight smoke | `bash scripts/tests/hooks/test_preflight.sh` | all PASS |
| Quality weakness gate wired | `bash scripts/tests/hooks/test_prforge_regressions.sh` | tests 16–18 cover all paths |
| Git state gate wired | `bash scripts/tests/hooks/test_prforge_regressions.sh` | tests 19–20 cover pr_approve.py |
...
| Certify runner | `bash scripts/certify.sh` | Issue #1 — creates this script |

**Current status:** PARTIAL — tests pass, hooks smoke, gates mechanically wired and regression-tested;
certify.sh runner (Issue #1) not yet created.

**Remaining gap:** Create `scripts/certify.sh` (Issue #1). Run it and capture output to
`docs/level0-certification-run.txt`.
...
**Current status:** BLOCKED — awaiting explicit user approval.
