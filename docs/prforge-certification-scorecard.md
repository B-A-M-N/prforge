...
| memory record in SQLite | ⚠️ | Degraded — `save-postmortem` schema mismatch; `memory.db` exists |
| `verify_level2_run.py` | ✅ | 22/22 checks CERTIFIED |

**Simulation script:** `scripts/tests/level1/simulate_full_run.sh`

**Known limitation:** `memory_ledger.py save-postmortem` exits 1 (SQLite schema mismatch between
the postmortem writer and the ledger's expected format). Memory runs in degraded mode — `memory.db`
is created but the postmortem record may not be fully indexed. Tracked as a separate issue.
